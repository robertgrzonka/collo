...
module.exports = {
  add(name, value) {
    if (this.colors) {
      if (this.config.get(`colors.${name}`)) {
        throw new Error(
          `This color already exists. If you want to overwrite it use ${chalk.bold.red(
            "collo.edit()"
          )}`
        );
      } else {
  ...
  edit(name, value) {
    if (this.colors && !this.config.get(`colors.${name}`)) {
      throw new Error(
        `This color doesn't exist. If you want to add new collor use ${chalk.bold(
          "collo.add()"
        )}`
      );
    } else {
  ...
  delete(name) {
    if (!this.colors[name]) {
      throw new Error(
        `This color doesn't exist. Use ${chalk(
          "collo.colors"
